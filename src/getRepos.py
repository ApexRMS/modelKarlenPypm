    ...
    These are exactly the data made available through his online ipypmm interface
 	...
 	1) Pypmca* (the visible project-level datasheets/lookup tables):
		...
		b) PypmcaCrosswalk - in order to compare models, we have a list of standard variable/population names to be used
 			...
 			'infected_v') to Ssim standarrd variable names ('Infected (Variants) - Daily') and vice versa.

		c) PypmcaData - a lut/complete list of all data sources available (stratified by age, sex, region), which may include
 			overlapping datasets (such as 'Canada - BC' and 'BC - All'). sometimes these overlapping sets come from different
 			sources (for ex., the UofT API and the BC CDC respectively, for the previuos example). the names also will overlap,
 	...
 	3) CrosswalkFile - the crosswalk lut mentioned in 1b) will also be written as a CSV file that can be amended by the user, should
		there be mistakes. currently (as of 27 April 2020) the crosswalk file is autogenerated through string parsing of the Karlen
		population names, but if there are mistaked, they can be corrected, and that file can then be referenced in every future
		transformer (since the project-level datasheets can't be erased from, only appended to).

 	4) DataDropdown - 1c) mentions that, to capture the multiple data sets pertinent to a single jurisdiction), aources uniquely
		identified by their country+region pair will be given the same names. this table just takes the fancy names, removes the
		duplicate fancy jurisdiction names and saves to a project-level datasheet to create a dropdown menu with no duplicates
...
'''
    ...
    3) each .pypm model object is stripped and 'default' parameter values taken (each model comes prefit, but in line with the online interface, the
    ...
    be added on the fly as specific models are chosen by the user, and variables will be added when selected in running the getExepctations transformer
'''
...
for country in countryList:
    ...
    for modelName in modelList:
        ...
        '''
 			...
 			this allows the user to choose which variables to adjust during the fitting step, and which ones to hold constant during the prcoess
		'''
...
for karlenCountry in karlenSources.keys():
    ...
    for karlenRegion in dataDescription['regional_data'].keys():
        ...
        if karlenCountry == 'Germany_age':
            ...
            fancyRegion = headerFile.germanStateName(fancyRegion)
        ...
        '''
            ...
            will all get the fancyRegion None, so that the fancy name of the regions would be multiple copies of "Canada - British Columbia"
            (the karlenCountry variable gave us the name of the state/province, and it's easy to find the country)
        '''
        ...
        if fancyRegion == None:
         ...
         # if there is a fancy region name, paste it. for example, "Canada - British Columbia - Fraser", "Germany - Nordrhein-Westfalen"
            ...
            # since Westfalhen isn't a subregion of North Rhine, as it would appear when charting
...
tempSources = allSources[-allSources.Region.isin(pypmcaData.Region)]
